...
public class TwoKeyDictionary<TKey1, TKey2, TValue> : IEnumerable
{
	...
	{
		if (key1 == null || key2 == null || ContainsKey1(key1) || ContainsKey2(key2)) return false;
	...
	{
		if (!(ContainsKey1(key1) || ContainsKey2(key2))) return false;
	...
	{
		if (!ContainsKey2(key2)) return false;
    ...
    /// <summary>
    ///    Removes the value associated with the outer key. If the outer key is not found, nothing happens. If the outer key is
    ///    found, the inner key must also exist and is also removed.
    /// </summary>
    /// <param name="key1"></param>
    public bool RemoveByKey1(TKey1 key1)
	{
		if (!m_dic1.TryGetValue(key1, out var tmp_key2)) return false;
    ...
    /// <summary>
    ///    Removes the value associated with the inner key. If the inner key is not found, nothing happens. If the inner key is
    ///    found, the outer key must also exist and is also removed.
    /// </summary>
    /// <param name="key2"></param>
    public bool RemoveByKey2(TKey2 key2)
	{
		if (!m_dic2.ContainsKey(key2)) return false;

		var tmp_key1 = m_dic1.First(kvp => kvp.Value.Equals(key2)).Key;
	...
	public override string ToString()
	{
		...
		foreach (var kvp in m_dic1) sb.AppendLine($"Key1: {kvp.Key}, Key2: {kvp.Value}, Value: {m_dic2[kvp.Value]}");
		return sb.ToString();
	}
